...
from garage.torch import (as_torch_dict, global_device, soft_update_model,
                          torch_to_np, as_torch)
import wandb
from src.replay_buffer import PrioritizedReplayBuffer, ReversePPPathBuffer,\
    HReversePPPathBuffer, UniformReversePPPathBuffer, ForwardPPPathBuffer
...
class TD3(RLAlgorithm):
    ...
    def __init__(
        ...
        self.use_custom_sampling_pdist = use_custom_sampling_pdist
        ...
        self.use_priority_buffer = isinstance(
            self._replay_buffer, PrioritizedReplayBuffer)
        self.use_replay_DPP_buffer = isinstance(
            self._replay_buffer, (ReversePPPathBuffer,
                                  HReversePPPathBuffer,
                                  UniformReversePPPathBuffer,
                                  ForwardPPPathBuffer,))
    ...
    def update_sampling_scheme(self):
        timesteps = self._replay_buffer.sample_all_timesteps()
        qf_loss, y, q = tuple(v.cpu().numpy()
                              for v in self._optimize_policy(timesteps, False)[:-1])
        imp = abs(y-q)
        self._replay_buffer.update_pdist(np.squeeze(imp))
        if self.use_replay_DPP_buffer:
            self._replay_buffer.update_starts(self._grad_steps_per_env_step)
        ...
        for ep in trainer.step_epochs():
            for cycle in range(self._steps_per_epoch):
                ...
                if not self.use_priority_buffer:
                    ...
                    episodes = trainer.step_episode
                    err = []
                    for eps in episodes.split():
                        qf_loss, y, q = tuple(v.cpu().numpy()
                                              for v in self._optimize_policy(eps, False)[:-1])
                        err.append(abs(y-q).item())
                    self._replay_buffer.add_episode_batch(episodes, err)

                # Update after warm-up steps.
                if trainer.total_env_steps >= self._update_after:
                    if self.use_custom_sampling_pdist:
                        self.update_sampling_scheme()
    ...
    # pylint: disable=invalid-unary-operand-type
    def _optimize_policy(self, samples_data, grad_step_timer, grads=True):
        ...
        """
        try:
            rewards = samples_data['rewards'].to(
                global_device()).reshape(-1, 1)
            ...
            rewards = as_torch(samples_data.rewards).to(
                global_device()).reshape(-1, 1)
            actions = as_torch(samples_data.actions).to(
                global_device())
            terminals = as_torch(samples_data.terminals).to(
                global_device()).reshape(-1, 1)
            try:
                next_observations = as_torch(samples_data.next_observations).to(
                    global_device())
                observations = as_torch(samples_data.observations).to(
                    global_device())
            except:
                observations = as_torch(self._env_spec.observation_space.flatten_n(samples_data.observations)).to(
                    global_device())
                next_observations = as_torch(self._env_spec.observation_space.flatten_n(samples_data.next_observations)).to(
                    global_device())
